...
use crate::ir::{IrDesign, LogicVec};
        ...
        if val.width <= 4 {
            ...
            if val.width == 1 {
                match val.bits.first() {
                    Some(crate::ir::LogicVal::Zero) => "0".to_string(),
                    Some(crate::ir::LogicVal::One) => "1".to_string(),
                    Some(crate::ir::LogicVal::X) => "x".to_string(),
                    Some(crate::ir::LogicVal::Z) => "z".to_string(),
                    None => "x".to_string(),
                }
            } else {
