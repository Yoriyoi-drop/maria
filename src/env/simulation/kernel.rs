...
use crate::simulator::{SimulationEngine, SimulationLimit};
...
impl SimulationKernel {
    ...
    pub fn build(design: IrDesign, opts: &KernelOptions) -> SimulationEngine {
        ...
        engine.debug_mode = if opts.deep_debug {
            crate::simulator::DebugMode::DeepDebug
        } else {
            crate::simulator::DebugMode::Normal
        };
    ...
    pub fn run(
        design: IrDesign,
        opts: &KernelOptions,
    ) -> Result<(SimulationEngine, u64), crate::error::SimError> {
