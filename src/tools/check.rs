...
use crate::tools::{collect_targets, section, kv};
...
fn analyze_includes(path: &Path, all_files: &[PathBuf]) -> Result<IncludeInfo, SimError> {
    let src = std::fs::read_to_string(path)
        .map_err(|e| SimError::with_diag(crate::diagnostics::DiagCode::IoError, format!("{}: {}", path.display(), e)))?;
...
fn scan_design(
    ...
    let (design, session) = crate::tools::open_project(targets, &[], &[], None)?;
    let mut problem = 0usize;

    let mods: HashMap<crate::intern::Symbol, _> =
        design.modules.iter().map(|m| (m.name, m.clone())).collect();

    if do_deps {
        section("Dependency Check");
        let mut known: HashSet<crate::intern::Symbol> = mods.keys().copied().collect();
...
fn collect_mod_instances(items: &[crate::ast::types::ModuleItem], out: &mut Vec<crate::intern::Symbol>) {
...
fn collect_gen_instances(gi: &GenerateItem, out: &mut Vec<crate::intern::Symbol>) {
    match gi {
