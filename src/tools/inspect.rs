...
use crate::ast::types::{GenerateItem, Module, ModuleItem, PortDirection, TypedefDecl};
use crate::frontend::compile_session::CompileSession;
use crate::frontend::module_index::EntryKind;
use crate::intern::Symbol;
use crate::tools::{open_project, section, kv};
...
pub fn run(args: &InspectArgs) -> Result<(), crate::error::SimError> {
    ...
    match cmd {
        ...
        "deps" => deps(&design),
        other => Err(crate::error::SimError::with_diag(
...
fn module_map(design: &crate::ast::types::Design) -> HashMap<Symbol, Module> {
    design
        .modules
        .iter()
...
fn instantiated_set(design: &crate::ast::types::Design) -> HashSet<Symbol> {
...
/// Statistik project.
fn stats(design: &crate::ast::types::Design, session: &CompileSession, json: bool) -> Result<(), crate::error::SimError> {
    ...
    for p in &design.packages {
        parameters += p.items.iter().filter(|i| matches!(i, crate::ast::types::PackageItem::Param(_))).count();
        typedefs += p.items.iter().filter(|i| matches!(i, crate::ast::types::PackageItem::Typedef(_))).count();
    }
...
/// Daftar package.
fn packages(design: &crate::ast::types::Design) -> Result<(), crate::error::SimError> {
    section("Packages");
    ...
    for p in pkgs {
        ...
        let n_typedefs = p
            .items
            .iter()
            .filter(|i| matches!(i, crate::ast::types::PackageItem::Typedef(_)))
            .count();
        let n_params = p
            .items
            .iter()
            .filter(|i| matches!(i, crate::ast::types::PackageItem::Param(_)))
            .count();
        let n_classes = p
            .items
            .iter()
            .filter(|i| matches!(i, crate::ast::types::PackageItem::Class(_)))
            .count();
        println!(
...
/// Daftar class.
fn classes(design: &crate::ast::types::Design) -> Result<(), crate::error::SimError> {
    section("Classes");
...
fn interfaces(design: &crate::ast::types::Design) -> Result<(), crate::error::SimError> {
    section("Interfaces");
...
fn deps(design: &crate::ast::types::Design) -> Result<(), crate::error::SimError> {
