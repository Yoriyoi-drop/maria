...
fn check_module_item<'a>(item: &'a MItem, ctx: &'a Ctx<'a>, scope: &mut Scope<'a>) -> Result<(), MvError> {
    match item {
        ...
        MItem::Inst {
            ...
            for c in conns {
                match c {
                    Conn::Named { expr: Some(e), .. } => check_expr(e, ctx, scope, 0)?,
                    Conn::Named { expr: None, .. } => {}
                    Conn::Positional(e) => check_expr(e, ctx, scope, 0)?,
                }
            }
...
fn check_type(ty: &MvType, ctx: &Ctx, depth: usize) -> Result<(), MvError> {
    ...
    match ty {
        ...
        MvType::Signed(inner) => check_type(inner, ctx, depth + 1)?,
        MvType::Array(inner, _) => check_type(inner, ctx, depth + 1)?,
        _ => {}
    }
    Ok(())
}
