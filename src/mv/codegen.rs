...
fn emit_module_kw(out: &mut String, m: &Module, kw: &str, iface_names: &[&str]) {
    ...
    if !m.params.is_empty() {
        ...
        let params: Vec<String> = m
            ...
            .map(|p| {
                let ty = match &p.ty {
                    Some(t) if matches!(t, MvType::Named(s, ..) if s == "type") => {
                        format!("parameter type {} = {}", p.name, p.default.as_ref().map(emit_expr).unwrap_or_default())
                    }
                    Some(t) => format!("parameter {} {} = {}", emit_type(t), p.name, p.default.as_ref().map(emit_expr).unwrap_or_default()),
                    None => format!("parameter {} = {}", p.name, p.default.as_ref().map(emit_expr).unwrap_or_default()),
                };
                format!("    {ty}")
            })
            .collect();
    ...
    for item in &m.items {
        match item {
            ...
            MItem::GenFor { var, from, to, body } => {
                ...
                for item in body {
                    emit_module_item_at(out, 2, item);
                }
                line(out, 1, "end");
...
fn emit_module_item_at(out: &mut String, indent: usize, item: &MItem) {
    match item {
        ...
        MItem::GenFor { var, from, to, body } => {
            ...
            for i in body {
                emit_module_item_at(out, indent + 2, i);
            }
...
"#;
    ...
    fn codegen_interface_only_triggers_svh() {
        // F26 fix review: file dengan HANYA interface (tanpa package/typedef)
        // tetap menghasilkan .svh + .sv meng-include-nya.
        let src = "interface bus {\n sig a : logic[7:0]\n modport m { in a }\n}\n";
        let out = generate(&parse(src).unwrap(), "bus");
        assert!(out.svh.contains("interface bus;"), "svh: {}", out.svh);
        assert!(out.svh.contains("`endif"));
        assert!(out.sv.contains("`include \"bus.svh\""));
    }
