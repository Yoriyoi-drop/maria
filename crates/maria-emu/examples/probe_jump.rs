...
fn main() {
    ...
    let mk = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(8_500_000);
    let extra: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(400_000);
    ...
    for _ in 0..mk {
        if cpu.halted { println!("HALT {:?}", cpu.halt_reason); return; }
    ...
    for i in 0..extra {
        ...
        if hi_now && hi == pc {
            println!("  [BIG-JUMP] step={} pc=0x{:08x} [0x{:02x} 0x{:02x} 0x{:02x} 0x{:02x}]",
                mk + i, pc, b0, b1, b2, b3);
        }
        ...
        if zero_now && !in_zero {
            ...
            for (j, p) in ring.iter().enumerate() {
                ...
                println!("      [{j}] step={} pc=0x{:08x} [0x{:02x} 0x{:02x} 0x{:02x}]",
                    mk + i - ring.len() + j, p, bb, bb1, bb2);
            }
        }
        ring[(i % 6) as usize] = pc;
        if log {
            println!("  step={} pc=0x{:08x} [0x{:02x} 0x{:02x} 0x{:02x} 0x{:02x}]",
                mk + i, pc, b0, b1, b2, b3);
        }
        let _ = (b0, b1, b2, b3);
        cpu.step(&mut mem).unwrap();
