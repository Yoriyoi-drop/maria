...
fn main() {
    ...
    m.add(RamRegion::new(Symbol::intern("ram"), 0x0, 0x100000, RegionKind::Ram, false).unwrap()).unwrap();
    ...
    gdt[0] = 0xff; gdt[1] = 0xff; gdt[2] = 0x00; gdt[3] = 0x00; // seg0 base 0
    gdt[4] = 0x00; gdt[5] = 0x9a; gdt[6] = 0xcf; gdt[7] = 0x00;
    gdt[8] = 0xff; gdt[9] = 0xff; gdt[10] = 0x00; gdt[11] = 0x00; // seg1 base 0
    gdt[12] = 0x00; gdt[13] = 0x92; gdt[14] = 0xcf; gdt[15] = 0x00;
    cpu.gdt_cache = gdt;
    cpu.cs = 0x8;
    cpu.ip = 0x1000;
    for _ in 0..10 {
        ...
        println!("step pc=0x{pc:08x} eax=0x{:08x} halted={}", cpu.r32(0), cpu.halted);
    }
    println!("mem[0x2000] = 0x{:08x} (expect 0)", m.read(0x2000, 4).unwrap_or(0xffff));
}
