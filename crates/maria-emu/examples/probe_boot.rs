...
fn main() {
    ...
    for i in 0..n {
        ...
        *hist.entry(pc).or_insert(0) += 1;
        ...
        if i % sample == 0 {
            ...
            let rel = prev
                .map(|p| pc.wrapping_sub(p) as i64 / sample as i64)
                .unwrap_or(0);
            println!(
    ...
    for (pc, c) in v.iter().take(12) {
        let bs: Vec<u8> = (0..4)
            .map(|k| mem.read(*pc + k, 1).unwrap_or(0) as u8)
            .collect();
        println!(
            "pc=0x{:08x} count={} [{:02x} {:02x} {:02x} {:02x}]",
            pc, c, bs[0], bs[1], bs[2], bs[3]
        );
    }
}
