...
fn main() {
    ...
    for a in (esp.saturating_sub(0x40))..(esp + 0x20) {
        ...
        if (a - esp.saturating_sub(0x40) + 1) % 16 == 0 { s.push('\n'); }
    }
    ...
    println!("gdt_base=0x{:08x} limit=0x{:04x} pmode={} cr0=0x{:08x} cs=0x{:04x} ss=0x{:04x}",
        cpu.gdt_base, cpu.gdt_limit, cpu.pmode, cpu.cr[0], cpu.cs, cpu.ss);
    for (name, base) in [("out0x100000", 0x100000u64), ("out0x101000", 0x101000), ("out0x202000", 0x202000), ("probtab_0x10c498", 0x10c498), ("in_0x8d10", 0x8d10), ("pc_region_0x3000", 0x3000), ("pc_region_0x8000", 0x8000), ("core_at_0x8200", 0x8200)] {
        let mut s = String::new();
        for a in base..base + 0x100 {
            s.push_str(&format!("{:02x} ", m.read(a, 1).unwrap_or(0)));
            if (a - base + 1) % 16 == 0 { s.push('\n'); }
        }
