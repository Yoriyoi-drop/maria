...
fn main() {
    ...
    for lba in 16..60u32 {
        ...
        println!("LBA {} type={} id={:?} desc='{}'", lba, t, String::from_utf8_lossy(id), desc);
        if t == 0 { // Boot Record
            // bytes 0x47..0x4A = boot catalog LBA
            let cat_lba = buf[0x47] as u64 | (buf[0x48] as u64)<<8 | (buf[0x49] as u64)<<16 | (buf[0x4A] as u64)<<24;
            ...
            loop {
                ...
                let t = e[0];
                if t == 0xff { break; } // terminator
                if t == 0x88 { // boot info / section
                    let media = e[1];
                    let seg = e[2] as u16 | (e[3] as u16) << 8;
                    let systs = e[4];
                    let lba = e[8] as u64 | (e[9] as u64)<<8 | (e[10] as u64)<<16 | (e[11] as u64)<<24;
                    let count = e[6] as u64 | (e[7] as u64)<<8;
                    println!("  Boot section: media={} seg=0x{:04x} systs=0x{:02x} lba={} len512={}", media, seg, systs, lba, count);
                } else {
            ...
            // PVD — sector size etc.
            println!("  PVD: volsize={} (2048-blocks), filesystem entry at LBA {}", 
                buf[0x50] as u64 | (buf[0x51] as u64)<<8 | (buf[0x52] as u64)<<16 | (buf[0x53] as u64)<<24,
                buf[0x9C] as u64 | (buf[0x9D] as u64)<<8 | (buf[0x9E] as u64)<<16 | (buf[0x9F] as u64)<<24);
        }
    }
}