...
//!
//! Liberty adalah format library sel standar industri (Synopsys). Mivon
...
pub fn load_mdb(path: &Path) -> std::io::Result<LibertyLibrary> {
    ...
    for line in text.lines() {
        let t = line.trim();
        if t.is_empty() || t.starts_with("MIVON-LIB") {
            continue;
        }
...
"#;
    ...
    fn mdb_roundtrip() {
        let lib = parse_liberty(SAMPLE).expect("parse");
        let path = std::env::temp_dir().join("mivon_test_generic.libmdb");
