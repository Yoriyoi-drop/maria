...
//! These tests validate Mivon's architecture at scale.
...
fn test_stress_parallel_compile_session() {
    ...
    let dir = std::env::temp_dir().join("mivon_stress_session");
...
fn test_stress_mmap_io() {
    use mivon_compiler::frontend::io::MmapFile;
    let dir = std::env::temp_dir().join("mivon_mmap_stress");
    let _ = std::fs::create_dir_all(&dir);
...
fn test_stress_incremental_tracker() {
    use mivon_compiler::scheduler::dag::DependencyGraph;
    use mivon_compiler::scheduler::incremental::IncrementalTracker;
    use mivon_compiler::scheduler::work_stealing::Task;
    use std::path::Path;
