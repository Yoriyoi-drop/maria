...
//! deterministik mivon-timing. Kriteria fase 5: "WNS/TNS/critical path benar
...
fn rtl_to_netlist(source: &str) -> mivon_netlist::Netlist {
    ...
    mivon_synth::tech_map(&sir_opt, &mivon_tech::FpgaX7Arch).netlist
}

#[test]
fn phase5_alu_timing_matches_manual() {
    let nl = rtl_to_netlist(&example("alu.sv"));
    let c = mivon_timing::Constraint {
        clocks: vec![mivon_timing::ClockSpec {
            name: "clk".into(),
            period_ns: 10.0,
        }],
        input_delay_ns: 2.0,
    ...
    let r = mivon_timing::analyze(&nl, &c, &mivon_timing::TimingOptions::default());
    ...
    let a = mivon_timing::estimate_area(&nl);
...
fn phase5_constraint_false_path_skipped() {
    ...
    let c = mivon_timing::parse_constraints(text);
