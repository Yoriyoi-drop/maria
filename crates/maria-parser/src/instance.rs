...
impl Parser {
    ...
    fn parse_param_block(
        ...
        (
            std::collections::HashMap<Symbol, maria_ast::Expr>,
            std::collections::HashMap<Symbol, maria_ast::DataType>,
        ),
        SimError,
    > {
        let mut param_assigns: std::collections::HashMap<Symbol, maria_ast::Expr> =
            std::collections::HashMap::new();
        let mut type_param_assigns: std::collections::HashMap<Symbol, maria_ast::DataType> =
            std::collections::HashMap::new();
    ...
    pub(crate) fn parse_gate_primitive(&mut self) -> Result<GatePrimitive, SimError> {
        ...
        if self.peek() == &Token::Hash {
            ...
            if self.peek() == &Token::LParen {
                ...
                self.expect(Token::RParen)?;
                delay = Some(maria_ast::types::Delay {
                    rise,
                    fall,
                    turnoff,
                });
