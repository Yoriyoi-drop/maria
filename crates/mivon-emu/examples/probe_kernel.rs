...
use mivon_emu::cpu::x86::{FileDisk, X86Cpu};
...
fn main() {
    ...
    for _ in 0..40_000_000 {
        ...
        if in_loop && loop_count < 12 {
            ...
            for off in 0..16usize {
                s.push_str(&format!(
                    "{:02x} ",
                    m.read((pc as usize + off) as u64, 1).unwrap_or(0)
                ));
            }
        ...
        if n % 500 == 0 {
            println!(
                "step={n} pc=0x{pc:x} eax=0x{:08x} esp=0x{:08x}",
                cpu.r32(0),
                cpu.r32(4)
            );
        }
    ...
    println!(
        "HALT: {} steps={} halt_pc=0x{halt_pc:x}",
        cpu.halt_reason, n
    );
    let a = 0xc7d0usize;
    let mut s = String::new();
    for off in 0..80usize {
        s.push_str(&format!(
            "{:02x} ",
            m.read((a + off) as u64, 1).unwrap_or(0)
        ));
        if (off + 1) % 16 == 0 {
            s.push('\n');
        }
