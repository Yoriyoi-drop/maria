...
impl Corpus {
    ...
    /// (Maria HDL) + project real (cva6/openc910/opentitan bila ada). Dengan
    ...
    pub fn load(dir: Option<&Path>) -> Self {
        ...
        match dir {
            ...
            None => {
                ...
                let base = root.join("crates/maria-fuzz/fuzz/corpus");
    ...
    /// (ingin ukur seberapa rusak maria secara penuh).
...
pub fn is_real_seed_path(p: &Path) -> bool {
    let s = p.to_string_lossy();
    // Absolut: segmen proyek setelah root maria.
...
/// Workspace root maria: naik dari cwd sampai Cargo.toml dengan `[workspace]`.
fn workspace_root() -> PathBuf {
    ...
    // Fallback: pakai kasih dari env MARIA_ROOT atau cwd.
    std::env::var("MARIA_ROOT")
        .map(PathBuf::from)
