...
//!
//! Menyediakan sistem plugin untuk Mivon:
//! - Plugin manifest format (mivon-plugin.toml)
...
/// Plugin manifest — metadata dari mivon-plugin.toml.
...
impl PluginManifest {
    ...
    /// Check compatibility.
    pub fn is_compatible(&self, mivon_version: &str) -> bool {
        version_ge(mivon_version, &self.min_mivon_version)
    }
}
...
impl PluginRegistry {
    ...
    pub fn scan(&mut self) {
        ...
        if let Ok(entries) = std::fs::read_dir(&self.registry_dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.is_dir() {
                    let manifest_path = path.join("mivon-plugin.toml");
    ...
    pub fn install(&mut self, source_dir: &Path) -> Result<String, String> {
        let manifest_path = source_dir.join("mivon-plugin.toml");
...
mod tests {
    ...
    fn make_plugin_dir(dir: &Path, name: &str, version: &str, deps: &[&str]) {
        ...
        let toml = format!(
            "name = \"{}\"\nversion = \"{}\"\ndescription = \"test plugin\"\nauthor = \"test\"\nmin_mivon_version = \"0.1.0\"\nfeatures = [\"simulation\"]\ndependencies = [{}]\n",
            name,
            version,
            deps_toml.join(", ")
        );
        std::fs::write(plugin_dir.join("mivon-plugin.toml"), toml).unwrap();
    }
