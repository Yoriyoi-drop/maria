...
mod tests {
    ...
    fn test_heartbeat() {
        let mut server = LicenseServer::new(300);
        server.register_pool("mivon-sim", 1);
        let id = server.checkout("mivon-sim", "alice", "host1").unwrap();
    ...
    fn test_summary() {
        let mut server = LicenseServer::new(300);
        server.register_pool("mivon-sim", 4);
        server.register_pool("mivon-formal", 2);
