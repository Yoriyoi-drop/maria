...
//! dalam satu workspace. File: `.mivon/workspace.toml`
//!
//! Format:
//! ```toml
//! [[projects]]
...
/// List all .mivon project files in a directory tree.
pub fn discover_projects(root: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(entries) = std::fs::read_dir(root) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                // Check for .mivon file in subdirectory
                let mivon_file = path.join("proj.mivon");
                if mivon_file.exists() {
                    projects.push(mivon_file);
                }
                ...
                if let Ok(sub_entries) = std::fs::read_dir(&path) {
                    for sub in sub_entries.flatten() {
                        ...
                        if sub_path.is_dir() {
                            let sub_mivon = sub_path.join("proj.mivon");
                            if sub_mivon.exists() {
                                projects.push(sub_mivon);
                            }
                        }
                    }
                }
