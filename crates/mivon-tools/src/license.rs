...
//!
//! Format license key: `MIVON-XXXX-XXXX-XXXX-XXXX`
//! Struktur internal: vendor/product/expiry/feature_flags/hmac_signature.
//!
//! Contoh penggunaan:
//! ```rust
//! use mivon_tools::license::LicenseManager;
//!
...
impl LicenseManager {
    ...
    pub fn generate_key(&self, tier_name: &str, expiry_days: u32) -> String {
        ...
        // Key format: MIVON-TTTTTTTT-RRRRDDDD-TTEEEE-FFFF
    ...
    pub fn validate(&self, key: &str) -> Result<LicenseKey, String> {
        // Check format
        if !key.starts_with("MIVON-") {
            return Err("invalid prefix (expected MIVON-)".to_string());
        }

        let parts: Vec<&str> = key.split('-').collect();
        if parts.len() != 5 {
            return Err("invalid format (expected MIVON-XXXX-XXXX-XXXX-XXXX)".into());
        }

        // Parse parts: MIVON-XXXXXXXX-XXXXXXXX-TTEEEE-FFFF
...
mod tests {
    ...
    fn test_invalid_format() {
        ...
        assert!(lm.validate("MIVON-1234").is_err());
    }
