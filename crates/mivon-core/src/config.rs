//! Mivon Config — loader file konfigurasi TOML (`configs/*.toml`).
//!
...
//!
//! Dipakai via flag CLI `--config <path>`; tanpa flag, Mivon memuat
...
/// Seluruh konfigurasi Mivon. Field opsional — nilai yang tidak ada di file
/// dibiarkan `None` (fallback ke default CLI/kode).
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(default)]
pub struct MivonConfig {
    #[serde(default)]
...
impl MivonConfig {
    ...
    pub fn load(path: &str) -> Result<MivonConfig, String> {
    ...
    pub fn load_auto(explicit: Option<&str>) -> Result<MivonConfig, String> {
