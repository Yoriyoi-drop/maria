...
use mivon_simulator::simulator::{SimulationEngine, SimulationLimit};
...
impl SimulationKernel {
    ...
    pub fn build(design: IrDesign, opts: &KernelOptions) -> SimulationEngine {
        ...
        engine.debug_mode = if opts.deep_debug {
            mivon_simulator::simulator::DebugMode::DeepDebug
        } else {
            mivon_simulator::simulator::DebugMode::Normal
        };
    ...
    pub fn run(
        design: IrDesign,
        opts: &KernelOptions,
    ) -> Result<(SimulationEngine, u64), mivon_core::error::SimError> {
