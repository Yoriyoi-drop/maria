...
/// Nilai dikumpulkan dari: default → file TOML/JSON → environment (`MIVON_*`)
...
impl ConfigContext {
    ...
    pub fn new(inner: MivonConfig) -> Self {
        ConfigContext {
            inner,
    ...
    fn from_file(inner: MivonConfig, path: PathBuf) -> Self {
        ConfigContext {
            inner,
    ...
    /// Bangun ConfigContext dari `MivonConfig` yang sudah di-load caller
    ...
    pub fn from_loaded(inner: MivonConfig, explicit: Option<&str>) -> Self {
        match explicit {
    ...
    /// `configs/compiler.toml` bila ada, lalu override `MIVON_*`.
        ...
        let mut ctx = match explicit {
            ...
            None => {
                ...
                if default.exists() {
                    ...
                    ConfigContext::new(MivonConfig::default())
                }
            }
        };
    ...
    pub fn raw(&self) -> &MivonConfig {
        &self.inner
    }

    /// Mutasi terpandu (dipakai override environment/CLI).
    pub fn mutate<F: FnOnce(&mut MivonConfig)>(&mut self, f: F) {
        f(&mut self.inner);
    }
...
mod tests {
    ...
    /// Path relatif terhadap root project (cwd test = direktori crate,
    /// sedangkan `configs/compiler.toml` ada di root mivon).
    ...
    fn test_default_config() {
        let ctx = ConfigContext::new(MivonConfig::default());
    ...
    fn test_from_file_source() {
        let ctx = ConfigContext::from_file(MivonConfig::default(), PathBuf::from("x.toml"));
        assert_eq!(ctx.source(), ConfigSource::File);
        assert_eq!(ctx.source_path(), Some(Path::new("x.toml")));
    }

    #[test]
    fn test_load_auto_root_project_config() {
        // `configs/compiler.toml` di root project (cwd test = direktori crate,
        // jadi pakai path eksplisit via root_rel) → ter-load sebagai File.
        let root_cfg = root_rel("configs/compiler.toml");
        let ctx = ConfigContext::load_auto(Some(root_cfg.to_str().unwrap()))
            .expect("load_auto harus sukses");
        assert_eq!(ctx.source(), ConfigSource::File);
    ...
    fn test_load_auto_explicit_path() {
        let dir = std::env::temp_dir().join("mivon_env_cfg_test");
    ...
    #[test]
    fn test_mutate() {
        let mut ctx = ConfigContext::new(MivonConfig::default());
