//! Override config dari environment (`MIVON_*`).
//! Urutan kekuatan: CLI > Environment > file config > default.

use crate::env::config::ConfigContext;

/// Baca override `MIVON_*` dan terapkan ke config. Tidak error bila variabel
/// tidak ada / tidak valid — nilai tersebut diabaikan.
pub fn apply_env(ctx: &mut ConfigContext) {
    if let Some(v) = parse_usize(&std::env::var("MIVON_JOBS").unwrap_or_default()) {
        if v > 0 {
            ctx.mutate(|c| c.compiler.jobs = Some(v));
        }
    }
    if let Some(v) = parse_u64(&std::env::var("MIVON_MAX_TIME").unwrap_or_default()) {
        ctx.mutate(|c| c.simulation.max_time = Some(v));
    }
    if let Some(v) = parse_bool(&std::env::var("MIVON_FORCE_SIM").unwrap_or_default()) {
        ctx.mutate(|c| c.simulation.force_sim = Some(v));
    }
    if let Some(v) = parse_u8(&std::env::var("MIVON_OPT_LEVEL").unwrap_or_default()) {
        if v <= crate::env::config::defaults::MAX_OPT_LEVEL {
            ctx.mutate(|c| c.compiler.opt_level = Some(v));
        }
    }
    if let Some(v) = parse_bool(&std::env::var("MIVON_INCREMENTAL").unwrap_or_default()) {
