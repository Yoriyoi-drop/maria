...
pub fn validate(cfg: &MivonConfig) -> Vec<String> {
...
pub fn warn_invalid(cfg: &MivonConfig) {
...
mod tests {
    ...
    fn test_valid_default() {
        assert!(validate(&MivonConfig::default()).is_empty());
    }

    #[test]
    fn test_catches_bad_opt_level() {
        let mut cfg = MivonConfig::default();
