...
pub fn startup() -> Result<GlobalEnv, String> {
    // 1. Load Config (TOML/JSON via loader + override MIVON_* env).
...
fn build_env(config: ConfigContext, workspace: WorkspaceContext) -> Result<GlobalEnv, String> {
    ...
    // di `crates/<crate>/.mivon/` (salah tempat).
    ...
    // 5. Initialize Cache.
    let cache_root = workspace.root.join(".mivon").join("cache");
...
mod tests {
    ...
    /// Root project mivon (cwd test = direktori crate; configs/ ada di root).
    fn root_dir() -> std::path::PathBuf {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..")
    }

    /// Bangun env dari root project tanpa bergantung cwd test.
    fn env_from_root() -> GlobalEnv {
        let cfg_path = root_dir().join("configs/compiler.toml");
        let config = ConfigContext::load_auto(Some(cfg_path.to_str().unwrap()))
            .expect("load_auto harus sukses");
        let workspace = WorkspaceContext::open_in(&root_dir());
        for_cli(config, workspace).expect("for_cli tidak boleh gagal di root project")
    }

    #[test]
    fn test_startup_builds_env() {
        let env = env_from_root();
    ...
    #[test]
    fn test_cache_root_is_under_mivon() {
        let env = env_from_root();
        let root = &env.cache().root;
        assert!(root.to_string_lossy().contains(".mivon"));
    }
}
