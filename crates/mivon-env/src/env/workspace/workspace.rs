...
use mivon_compiler::frontend::discovery::{DiscoveryOptions, FileDiscovery};
...
impl WorkspaceContext {
    ...
    /// (mis. `cargo test -p mivon-env` di `crates/mivon-env`), workspace.root
    /// = direktori crate → `.mivon/` (MICD database) dibuat di dalam crate —
    ...
    /// Muat file proyek `.mivon`/`.f` dan daftarkan sebagai filelist +
...
mod tests {
    ...
    fn test_find_project_root_resolves_workspace_root() {
        // F37: dari subdir crate (`crates/mivon-env`), project root harus
        // resolve ke root workspace (Cargo.toml ber-[workspace] / .git),
        // BUKAN direktori crate — agar `.mivon/` tak dibuat di dalam crate.
        ...
        assert!(
            ...
            "root harus project root mivon, dapat: {root:?}"
        );
        assert!(
            !root.ends_with("crates/mivon-env") && !root.ends_with("mivon-env"),
            "root tidak boleh direktori crate: {root:?}"
        );
        // open() memakai root yang sama → .mivon di root project, bukan crate.
    ...
    fn test_open_in_creates_structure() {
        let dir = std::env::temp_dir().join("mivon_ws_test");
    ...
    fn test_discover_sources_dedup_sorted() {
        let dir = std::env::temp_dir().join("mivon_ws_discover");
        let _ = std::fs::create_dir_all(dir.join("rtl"));
    ...
    fn test_load_project_registers_incdir() {
        let dir = std::env::temp_dir().join("mivon_ws_proj");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join("p.mivon"), "+incdir+inc\nrtl/a.sv\n").unwrap();
        let mut ws = WorkspaceContext::open_in(&dir);
        ws.load_project(&dir.join("p.mivon")).unwrap();
