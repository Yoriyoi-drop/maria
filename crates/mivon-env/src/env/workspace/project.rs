...
/// File proyek `.mivon` — daftar file `.sv` (satu per baris, `#` komentar),
...
mod tests {
    ...
    fn test_project_file_load() {
        let dir = std::env::temp_dir().join("mivon_project_test");
    ...
    fn test_project_file_empty_is_err() {
        let dir = std::env::temp_dir().join("mivon_project_empty");
        let _ = std::fs::create_dir_all(&dir);
        let p = dir.join("empty.mivon");
