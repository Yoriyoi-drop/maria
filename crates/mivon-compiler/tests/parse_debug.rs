...
fn debug_nonansi_port_width() {
    ...
    // Jalur legacy (sama dengan mivon_api::compile_str yang dipakai synth test).
    let mut pp = mivon_parser::preprocessor::Preprocessor::new();
    let pre = pp.preprocess(src, None).unwrap();
    let mut lexer = mivon_parser::lexer::Lexer::new(&pre);
    ...
    let mut parser = mivon_parser::Parser::new(tokens, "<string>")
...
fn debug_parse_body_localparam() {
    ...
    // Tes lexing langsung pada baris $value$plusargs
    {
        ...
        for (label, toks) in [
            ("F-$", {
                let mut lex = mivon_compiler::frontend::FastLexer::new(line, "x");
                let mut t = Vec::new();
                loop {
            ...
            ("L-$", {
                let mut lex = mivon_parser::lexer::Lexer::new(line);
                let mut t = Vec::new();
                loop {
                    let (tok, l, c) = lex.next_token();
                    if tok == mivon_parser::lexer::Token::Eof {
    ...
    // Bandingkan token streams: cari divergensi pertama
    {
        ...
        loop {
            ...
            if tf == mivon_parser::lexer::Token::Eof {
    ...
    for (label, toks) in [
        ...
        ("LEGACY", {
            let mut lexer = mivon_parser::lexer::Lexer::new(&combined);
            let mut t = Vec::new();
            loop {
                let (tok, line, col) = lexer.next_token();
                if tok == mivon_parser::lexer::Token::Eof {
