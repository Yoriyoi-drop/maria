...
fn main() {
    ...
    let db_root =
        std::env::var("MP_ROOT").unwrap_or_else(|_| "/home/whale-d/mivon/.mivon/database".into());
    let store_root = Path::new(&db_root)
        .join("cache")
        .join(pid)
        .join("preprocess");
    let mut store = mivon_compiler::micd::cache::store::CategoryStore::open(
        &store_root,
        mivon_compiler::micd::cache::category::CacheCategory::Preprocess,
        0,
    );
