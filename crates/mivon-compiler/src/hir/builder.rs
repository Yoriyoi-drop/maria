...
impl HirBuilder {
    ...
    /// AST types (mivon_ast::Module) to HIR types.
    pub fn build_module(
        &mut self,
        name: Symbol,
        params: Vec<HirParam>,
    ...
    pub fn ast_dtype_to_hir(&self, dtype: &mivon_ast::types::DataType, scope: Symbol) -> HirType {
        self.type_system.ast_dtype_to_hir(dtype, scope)
    }

    /// Register typedefs with the type system.
    pub fn register_typedefs(&self, scope: Symbol, typedefs: &[mivon_ast::types::TypedefDecl]) {
