...
impl FileDiscovery {
    ...
    pub fn scan_file_list(path: &Path) -> Result<Vec<PathBuf>, String> {
        ...
        // Section header `[...]` di project file .mivon ([foreign], dll):
        ...
        // dengan mivon-api::read_project_file. Konvensi: file list dulu,
...
mod tests {
    ...
    fn test_scan_test_dir() {
        ...
        let result = FileDiscovery::scan_dir(root_rel("test"), &options);
    ...
    fn test_scan_file_list() {
        let dir = std::env::temp_dir().join("mivon_test_discovery");
    ...
    fn test_scan_file_list_skips_section_headers() {
        // Project file .mivon dengan section `[foreign]` / header lain —
        // header BUKAN path file; hanya baris .sv yang di-scan.
        let dir = std::env::temp_dir().join("mivon_test_discovery_sections");
        let _ = std::fs::create_dir_all(&dir);
        let list_path = dir.join("proj.mivon");
        std::fs::write(
            &list_path,
            "rtl/top.sv\n\n[foreign]\nvhpi = [\"libvhpi.so\"]\n\n[[emu.device]]\nname = \"u_uart\"\nmmio = 0x10000000\n",
        )
    ...
    #[test]
    fn test_checksum() {
        let dir = std::env::temp_dir().join("mivon_test_checksum");
        let _ = std::fs::create_dir_all(&dir);
