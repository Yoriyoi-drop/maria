...
use mivon_core::LogicVal;
use mivon_netlist::cell::{CellInstance, CellKind, PinConn};
use mivon_netlist::net::{NetId, Netlist, PortDir};
use mivon_sir::{ResetSpec, SirModule, SirNodeKind, SirRegister, SirValue};
use mivon_tech::TechArch;
...
impl<'a> TechMapper<'a> {
    ...
    fn build_node_bit(&self, node: &mivon_sir::SirNode, i: usize) -> BitFn {
        let sir = self.sir;
    ...
    fn barrel_shift(&self, node: &mivon_sir::SirNode) -> Vec<BitFn> {
        let w = node.width.max(1);
        let src = node.inputs[0];
    ...
    fn map_all(&mut self) {
        ...
        for p in &self.sir.inputs {
            ...
            let dir = match p.dir {
                mivon_sir::PortDir::Input => PortDir::Input,
                mivon_sir::PortDir::Inout => PortDir::Inout,
                mivon_sir::PortDir::Output => PortDir::Output,
            };
...
mod tests {
    ...
    /// SIR counter 8-bit (same shape as mivon-netlist tests) — clk, rst_n,
    /// ADD(count,1), FF.
    fn counter_sir() -> SirModule {
        ...
        let mk = |name: &str, dir: mivon_sir::PortDir, value: ValueId, width: usize| {
            mivon_sir::SirPort {
        ...
        m.inputs.push(mk("clk", mivon_sir::PortDir::Input, 0, 1));
        m.inputs.push(mk("rst_n", mivon_sir::PortDir::Input, 1, 1));
        m.outputs
            .push(mk("count", mivon_sir::PortDir::Output, 2, 8));
        m
    }
    ...
    fn wide_and_sir() -> SirModule {
        ...
        let mk = |name: &str, value: usize, width: usize| mivon_sir::SirPort {
            name: Symbol::intern(name),
            dir: mivon_sir::PortDir::Input,
            width,
            value,
        };
        for i in 0..8 {
    ...
    fn reduce_and_sir() -> SirModule {
        ...
        m.inputs.push(mivon_sir::SirPort {
            name: Symbol::intern("a"),
            dir: mivon_sir::PortDir::Input,
            width: 8,
            value: v,
        });
        m.outputs.push(mivon_sir::SirPort {
            name: Symbol::intern("y"),
            dir: mivon_sir::PortDir::Output,
            width: 1,
            value: out,
        });
        m
    ...
    fn emit_mapped_netlist_has_lut_and_carry_modules() {
        ...
        let mut v = mivon_netlist::emit_verilog(&tech_map(&counter_sir(), &arch).netlist);
        v.push_str(&mivon_netlist::emit_verilog(
    ...
    fn or_sir() -> SirModule {
        ...
        let mk = |name: &str, value: usize| mivon_sir::SirPort {
            name: Symbol::intern(name),
            dir: mivon_sir::PortDir::Input,
            width: 8,
            value,
