//! Resource guard untuk simulasi design besar (MIVON-SIM-34).
//!
...
//!
//! Ambang batas dikonfigurasi via env `MIVON_SIM_MEM_LIMIT_MB`. Nilai `0`
...
use mivon_core::diagnostics::{DiagCode, DiagLevel, Diagnostic};
...
impl SimResourceGuard {
    /// Bangun guard dari env `MIVON_SIM_MEM_LIMIT_MB` atau default yang
                ...
                let diag = Diagnostic::new(
                    ...
                    format!(
                        "simulasi melebihi batas memori {limit} MiB (RSS kini {rss} MiB) pada waktu {time} ns. \
                         ...
                         \x20 • naikkan batas via env `MIVON_SIM_MEM_LIMIT_MB=<lebih_besar>` bila RAM tersedia.",
                        limit = self.limit_mb,
                        rss = rss_mb
                    ),
                );
