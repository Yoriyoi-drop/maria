...
use mivon_ir::{LogicVal, LogicVec};
...
mod tests {
    ...
    fn test_spill_to_disk() {
        let dir = std::env::temp_dir().join(format!("mivon_sh_test_{}", std::process::id()));
    ...
    fn test_multi_signal_spill() {
        let dir = std::env::temp_dir().join(format!("mivon_sh_test2_{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
