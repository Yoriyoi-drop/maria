...
use mivon_ir::{IrDesign, LogicVal};
...
impl VcdWriter {
    ...
    fn write_header(&mut self, design: &IrDesign) -> Result<(), String> {
        self.write_raw(b"$version Mivon RTL Simulator v0.1.0 $end\n")?;
    ...
    pub fn enable_background(&mut self) -> Result<(), String> {
        ...
        let handle = std::thread::Builder::new()
            .name("mivon-vcd-writer".to_string())
            .spawn(move || {
...
fn vec_to_vcd(val: &mivon_ir::LogicVec) -> String {
...
mod tests {
    ...
    fn test_background_mode_writes_identical_output() {
        let dir = std::env::temp_dir().join("mivon_vcd_bg_test");
    ...
    fn test_background_flush_synchronizes() {
        let dir = std::env::temp_dir().join("mivon_vcd_bg_flush");
        let _ = std::fs::create_dir_all(&dir);
        let design = make_design();
    ...
    fn test_background_drop_finalizes_file() {
        let dir = std::env::temp_dir().join("mivon_vcd_bg_drop");
        let _ = std::fs::create_dir_all(&dir);
        let design = make_design();
    ...
    fn test_compressed_vcd_is_valid_gzip() {
        let dir = std::env::temp_dir().join("mivon_vcd_gzip_test");
        let _ = std::fs::create_dir_all(&dir);
        let design = make_design();
    ...
    fn test_compressed_vcd_new_compressed() {
        let dir = std::env::temp_dir().join("mivon_vcd_gzip_new");
        let _ = std::fs::create_dir_all(&dir);
        let design = make_design();
