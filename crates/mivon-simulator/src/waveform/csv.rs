...
use mivon_ir::{IrDesign, LogicVec};
        ...
        if val.width <= 4 {
            ...
            if val.width == 1 {
                match val.bits.first() {
                    Some(mivon_ir::LogicVal::Zero) => "0".to_string(),
                    Some(mivon_ir::LogicVal::One) => "1".to_string(),
                    Some(mivon_ir::LogicVal::X) => "x".to_string(),
                    Some(mivon_ir::LogicVal::Z) => "z".to_string(),
                    None => "x".to_string(),
                }
            } else {
...
mod tests {
    ...
    fn test_csv_new() {
        ...
        let dir = std::env::temp_dir().join("mivon_csv_test");
    ...
    fn test_csv_empty_signals() {
        ...
        let dir = std::env::temp_dir().join("mivon_csv_test2");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_csv_dump_and_flush() {
        let design = make_design();
        let dir = std::env::temp_dir().join("mivon_csv_test3");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_csv_disabled_does_not_write() {
        let design = make_design();
        let dir = std::env::temp_dir().join("mivon_csv_test4");
        let _ = std::fs::create_dir_all(&dir);
