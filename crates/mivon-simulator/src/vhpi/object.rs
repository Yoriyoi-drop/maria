...
//! yang relevan untuk adapter Mivon). Konstanta mengikuti `vhpi_user.h`
//! IEEE 1076-2008 — BUKAN versi Mivon sendiri (poin 1 masukan user:
...
/// Memetakan ke signal/port/module Mivon. Bila tidak ditemukan → NULL.
    ...
    with_vhpi_engine(|engine| {
        ...
        for (sig_id, sig) in engine.design.top.signals.iter().enumerate() {
            if sig.name.as_str() == name
                && matches!(
                    sig.kind,
                    mivon_ir::SignalKind::Input
                        | mivon_ir::SignalKind::Output
                        | mivon_ir::SignalKind::Inout
                )
            {
        ...
        vhpiDirection => match &obj.kind {
            VhpiObjectKind::Port(sig_id, _) => with_vhpi_engine(|e| {
                e.design
                    ...
                    .get(*sig_id)
                    .map(|s| match s.kind {
                        mivon_ir::SignalKind::Input => vhpiIn,
                        mivon_ir::SignalKind::Output => vhpiOut,
                        mivon_ir::SignalKind::Inout => vhpiInout,
                        _ => vhpiNoDirection,
                    })
...
/// vhpi_is_defined(kind) — apakah object kind di-support adapter Mivon.
