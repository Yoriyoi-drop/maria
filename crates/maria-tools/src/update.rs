//! `maria update` — auto-update Maria dari release resmi.
//!
...
//!   - Default eksplisit: tanpa perintah `maria update`, tidak ada yang berubah.
...
pub const DEFAULT_MANIFEST_URL: &str =
    "https://raw.githubusercontent.com/Yoriyoi-drop/maria/main/dist/latest.json";

const REPO: &str = "Yoriyoi-drop/maria";
...
pub struct UpdateArgs<'a> {
    /// `true` → hanya melapor (`maria update check`), tanpa mengubah apa pun.
    pub check_only: bool,
...
/// Jalankan `maria update`. Exit semantics via `SimError` (main.rs `exit_tool`).
pub fn run(args: &UpdateArgs) -> Result<(), SimError> {
    ...
    let target: (String, String, String) = if let Some(ver) = args.version {
        ...
        let url = format!(
            "https://github.com/{}/releases/download/v{}/maria",
            REPO, ver
        );
        (ver, url, sha)
    } else {
    ...
    if !newer(&target.0, &local) {
        println!("Maria sudah terbaru (v{})", local);
        return Ok(());
    }
    ...
    if args.check_only {
        ...
        println!("Jalankan `maria update` untuk memasang.");
        return Ok(());
    }
...
/// Versi binary (versi crate maria-tools saat build; seluruh workspace
/// mengikuti versi root `maria`).
...
fn exe_path(override_path: Option<&str>) -> PathBuf {
    match override_path {
        ...
        None => std::env::current_exe().unwrap_or_else(|_| PathBuf::from("maria")),
    }
}
...
/// SHA-256 artefak `maria` dari release v<ver> (via asset maria.sha256).
fn fetch_checksum_for(version: &str) -> Result<String, SimError> {
    let url = format!(
        "https://github.com/{}/releases/download/v{}/maria.sha256",
        REPO, version
    );
    let text = fetch_text(&url)?;
    let sha = text
...
/// Backup file exe ke `<dir>/maria.bak` (hapus backup lama dulu).
fn backup_current(exe: &Path) -> Result<(), SimError> {
    if !exe.exists() {
        return Ok(());
    }
    let dir = exe.parent().unwrap_or_else(|| Path::new("."));
    let bak = dir.join("maria.bak");
    if bak.exists() {
...
pub fn install_binary_bytes(exe: &Path, data: &[u8]) -> Result<(), SimError> {
    let dir = exe.parent().unwrap_or_else(|| Path::new("."));
    let tmp = dir.join(format!(".maria-update-{}.tmp", std::process::id()));
...
mod tests {
    ...
    fn install_dan_rollback_atomik() {
        let dir = std::env::temp_dir().join("maria-update-test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let exe = dir.join("maria");

        // instal pertama (tanpa backup sebelumnya)
        install_binary_bytes(&exe, b"#!/bin/sh\necho fake\n").unwrap();
        assert!(exe.exists());
        assert!(!dir.join("maria.bak").exists());
    ...
    fn fetch_text_file_url() {
        let dir = std::env::temp_dir().join("maria-update-test-manifest");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
