//! Sidebar Search — cari simbol design sesuai desain Mivon: "bukan sekadar
...
fn signals_ui(
    ui: &mut egui::Ui,
    info: Option<&CompileInfo>,
    design: Option<&mivon_ir::IrDesign>,
    q: &str,
    to_open: &mut Option<(PathBuf, Option<usize>)>,
    found_any: &mut bool,
) {
    ...
    let mut matched: Vec<(String, usize, String, Option<PathBuf>)> = Vec::new();
    {
        ...
        let mut add_mod = |owner: &str, m: &mivon_ir::IrModule| {
            for s in &m.signals {
...
fn parameters_ui(
    ...
    let Some(info) = info else { return };
    let matched: Vec<&crate::state::ParamRow> = info
        .param_index
        .iter()
        .filter(|p| {
...
fn macros_ui(
    ...
    let Some(info) = info else { return };
    let matched: Vec<&crate::state::MacroRow> = info
        .macro_index
        .iter()
...
fn instances_ui(
    ...
    let Some(info) = info else { return };
    let matched: Vec<&crate::state::InstanceRow> = info
        .instance_index
        .iter()
        .filter(|i| {
